...
> **Architecture Note**: As of v0.1.0, EATP is a standalone SDK (`pip install kailash[trust]`). Kaizen's `kaizen.trust` module is a **shim layer** that re-exports from the standalone package. Canonical code lives in the SDK source. For standalone SDK documentation, see `skills/26-eatp-reference/`.
...
**Canonical Source**: the SDK source
...
- **Canonical Source**: the SDK source (standalone SDK)
- **Kaizen Shims**: the package source
- **EATP Tests**: the SDK source (1324 tests)
...
- **Examples**: the SDK source (standalone), `examples/trust/` (Kaizen integration)
