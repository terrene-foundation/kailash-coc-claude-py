...
3. **Real Infrastructure Testing**:
   ...
   - Run `docker compose up -d && docker compose ps` before integration tests
...
- [ ] Docker setup: docker compose up -d && docker compose ps
...
- MUST run: `docker compose up -d && docker compose ps` before tests
